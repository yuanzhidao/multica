...
-- conversation. attempt is incremented; max_attempts and trigger_comment_id
-- are inherited. max_attempts, trigger_comment_id, and is_leader_task are
-- inherited so
...
SELECT
    ...
    CASE WHEN p.failure_reason = 'codex_semantic_inactivity' THEN NULL ELSE p.session_id END,
    CASE WHEN p.failure_reason = 'codex_semantic_inactivity' THEN NULL ELSE p.work_dir END,
    p.attempt + 1, p.max_attempts, p.id,
    COALESCE(p.failure_reason = 'codex_semantic_inactivity', FALSE),
    p.is_leader_task
