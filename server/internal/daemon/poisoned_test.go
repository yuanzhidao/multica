...
func TestClassifyResumeUnsafeTimeout(t *testing.T) {
	cases := []struct {
	...
	}{
		{
			...
			provider:   "codex",
			errMsg:     "codex semantic inactivity timeout after 10m0s without agent progress (last activity: tool-result:exec_command)",
			wantOK:     true,
		...
		},
		{
			...
			provider: "claude",
			errMsg:   "codex semantic inactivity timeout after 10m0s without agent progress",
			wantOK:   false,
		},
		{
			name:     "empty error",
